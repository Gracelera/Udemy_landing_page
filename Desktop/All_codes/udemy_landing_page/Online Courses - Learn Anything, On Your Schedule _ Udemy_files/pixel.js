...
})(
  {
                  ...
                  WordArray = (C_lib.WordArray = Base.extend({
                    init: function (words, sigBytes) {
                      (words = this.words = words || []),
                        (this.sigBytes =
                          null != sigBytes ? sigBytes : 4 * words.length);
                    },
                        ...
                        var rcache,
                          ...
                          r = function (m_w) {
                            ...
                            return function () {
                              ...
                              return (
                                (result /= 4294967296),
                                (result += 0.5) * (0.5 < Math.random() ? 1 : -1)
                              );
                            };
                          },
                          i = 0;
                  ...
                  C_algo =
                    ((C_lib.Hasher = BufferedBlockAlgorithm.extend({
                      ...
                      update: function (messageUpdate) {
                        return (
                          this._append(messageUpdate), this._process(), this
                        );
                      },
    ...
    5: [
      function (require, module, exports) {
        ...
        var setAndTestCookie = function (name, value, domain, expires) {
          ...
          return (
            "https:" === window.location.protocol && (cookie += ";secure"),
            (document.cookie = cookie),
            0 <= document.cookie.indexOf(name)
          );
        };
        (exports.setAndTestCookie = setAndTestCookie),
          (exports.setCookie = function (name, value, expiresDays) {
            var d = new Date(),
            ...
            if (
              ((domain =
                "." == domain.slice(-1) ? domain.slice(0, -1) : domain),
              (labels = domain.split(".")),
              1 == labels.length || stringIsInteger(labels[labels.length - 1]))
            )
              return setAndTestCookie(name, value, domain, expires), domain;
            for (
              domain = labels.pop(), domain = labels.pop() + "." + domain;
              !setAndTestCookie(name, value, domain, expires) && labels.length;
      ...
      { "./constants": 4, "./strings": 11, "./uuid": 12 },
    ],
    6: [
      function (require, module, exports) {
        exports.screenSize = function () {
          return (
            ...
            { sh: Math.max(width, height), sw: Math.min(width, height) }
          );
        };
      },
      {},
        ...
        var console = window.console || {},
          ...
          log = function (method) {
            return function (msg) {
              method in console &&
                method in prefixes &&
                console[method](prefixes[method] + ":" + msg);
            };
          };
    ...
    8: [
      function (require, module, exports) {
        ...
        (exports.sendInit = function (config, args) {
          sendMessage({ type: "Init", config: config, args: args });
        }),
    ...
    9: [
      function (require, module, exports) {
        ...
        rdt ||
          (logger.error(
            "Pixel was not initialized.Please ensure you have included the correct pixel script in your head tag"
          ),
          pixelHelperConnect.sendError("PixelWasNotInitialised"));
        var enableFirstPartyCookies = function () {
          var firstPartyCookie =
            cookie.getCookieV2() || cookie.createCookieV2();
          cookie.setCookie(
            CONSTANTS.UUID_COOKIE_NAME_V2,
            firstPartyCookie,
            CONSTANTS.UUID_COOKIE_EXPIRATION_DAYS
          ),
            (rdt.uuid = cookie.extractUuidFromCookieV2(firstPartyCookie));
          var clickId = getQueryParameter(
            window.location.search,
            CONSTANTS.CLICK_ID_NAME
          );
          clickId && !rdt.clickId && (rdt.clickId = clickId),
        ...
        function sendEvent(method) {
          ...
          switch (method) {
            case PIXEL_METHODS.INIT:
              return rdt.advertiserId
                ? (logger.warn("pixel has already been initialized"),
                  void pixelHelperConnect.sendError(
                    "PixelAlreadyBeenInitialised",
                    args
                  ))
                : ((rdt.advertiserId = args[1]),
                  (configArgs = ("object" == typeof args[2] && args[2]) || {}),
                  (screenSize = device.screenSize()),
                  (rdt.config = {
                    aaid: hasher.hashIfSet(configArgs.aaid, "aaid"),
                    em: hasher.hashIfSet(configArgs.email, "email"),
                    external_id: hasher.hashIfSet(
                      configArgs.externalId,
                      "externalId"
                    ),
                    idfa: hasher.hashIfSet(configArgs.idfa, "idfa"),
                    integration: integrations.getIntegrationProvider(
                      configArgs.integration
                    ),
                  ...
                  "useDecimalCurrencyValues" in configArgs
                    ? (rdt.useDecimalCurrencyValues =
                        !!configArgs.useDecimalCurrencyValues)
                    : (rdt.useDecimalCurrencyValues = !0),
                  enableFirstPartyCookies(),
                  void pixelHelperConnect.sendInit(rdt.config, args));
            ...
            case PIXEL_METHODS.TRACK:
              ...
              if (!event.isValidEventType(eventName))
                return (
                  logger.error(
                    eventName +
                      " is not a valid pixel event please check the reddit pixel documentation for a list of valid pixel event types"
                  ),
                  void pixelHelperConnect.sendError("InvalidEventName", args)
                );
              var metadataArgs = args[2] || {},
                validatedMetadata = event.validateMetadata(
                  eventName,
                  metadataArgs
                ),
                q = new QueryBuilder()
                  .addParams({ id: rdt.advertiserId, event: eventName })
                  .addParams(validatedMetadata);
              return (
                rdt.enableFirstPartyCookies &&
                  (rdt.clickId &&
                    rdt.clickId.length &&
                    q.addParams({ click_id: rdt.clickId }),
                  rdt.uuid &&
                    rdt.uuid.length &&
                    q.addParams({ uuid: rdt.uuid })),
      ...
      function (require, module, exports) {
        ...
        (QueryBuilder.prototype.addParams = function (params) {
          for (var key in params)
            this.isMetadata(key)
              ? (this.params[CONSTANTS.METADATA_PREFIX + key] = params[key])
          ...
          (exports.getQueryParameter = function (search, name) {
            if (!search) return "";
            for (
              var keyValues = search.slice(1).split("&"), i = 0;
              i < keyValues.length;
              i++
            ) {
    ...
    13: [
      function (require, module, exports) {
        ...
        exports.validateMetadata = function (eventName, unvalidatedEvent) {
          ...
          if (isRevenueEventType(eventName)) {
            for (var i = 0; i < CONSTANTS.REVENUE_METADATA_LIST.length; i++) {
              ...
              field !== CONSTANTS.EVENT_METADATA.VALUE_DECIMAL &&
                copyValidatedField(field, validatedEvent, unvalidatedEvent);
            }
            filterDependencies(validatedEvent);
          }
          return (
            isCustomEventType(eventName) &&
              copyValidatedField(
                CONSTANTS.EVENT_METADATA.CUSTOM_EVENT_NAME,
                validatedEvent,
                unvalidatedEvent
              ),
            copyValidatedField(
              CONSTANTS.EVENT_METADATA.PRODUCTS,
              validatedEvent,
              unvalidatedEvent
            ),
        ...
        (exports.arrayIncludes = arrayIncludes),
          (exports.isValidEventType = function (eventName) {
        ...
        var copyValidatedField = function (
            field,
            validatedEvent,
            unvalidatedEvent
          ) {
            field in unvalidatedEvent &&
              getValidator(field)(unvalidatedEvent[field]) &&
              (validatedEvent[field] = unvalidatedEvent[field]);
          },
          filterDependencies = function (validatedEvent) {
            CONSTANTS.EVENT_METADATA.CURRENCY in validatedEvent &&
              !(CONSTANTS.EVENT_METADATA.VALUE in validatedEvent) &&
              (validatedEvent[CONSTANTS.EVENT_METADATA.CURRENCY] = "");
          },
          normalizeMetadata = function (validatedEvent) {
            ...
            if (
              ...
              var givenValue = validatedEvent[CONSTANTS.EVENT_METADATA.VALUE],
                valueFieldName = rdt.useDecimalCurrencyValues
                  ? CONSTANTS.EVENT_METADATA.VALUE_DECIMAL
                  : CONSTANTS.EVENT_METADATA.VALUE;
              rdt.useDecimalCurrencyValues &&
                delete validatedEvent[CONSTANTS.EVENT_METADATA.VALUE],
                (validatedEvent[valueFieldName] = parseFloat(givenValue));
            }
            if (CONSTANTS.EVENT_METADATA.ITEM_COUNT in validatedEvent) {
              var itemCount =
                validatedEvent[CONSTANTS.EVENT_METADATA.ITEM_COUNT];
              validatedEvent[CONSTANTS.EVENT_METADATA.ITEM_COUNT] =
                parseFloat(itemCount);
            }
            CONSTANTS.EVENT_METADATA.TRANSACTION_ID in validatedEvent &&
              (validatedEvent[CONSTANTS.EVENT_METADATA.TRANSACTION_ID] =
                hasher.hashIfSet(
                  validatedEvent[CONSTANTS.EVENT_METADATA.TRANSACTION_ID],
                  CONSTANTS.EVENT_METADATA.TRANSACTION_ID
                ));
            var customEventField = CONSTANTS.EVENT_METADATA.CUSTOM_EVENT_NAME;
            validatedEvent[customEventField] = normalizeCustomEventName(
              validatedEvent[customEventField]
            );
            var productsField = CONSTANTS.EVENT_METADATA.PRODUCTS;
            validatedEvent[productsField] = normalizeProducts(
              validatedEvent[productsField]
            );
            for (
              var event = {}, i = 0;
              i < CONSTANTS.EVENT_METADATA_LIST.length;
              i++
            ) {
              var field = CONSTANTS.EVENT_METADATA_LIST[i];
              event[field] =
                field in validatedEvent ? validatedEvent[field] : "";
            }
            return event;
          },
          normalizeProducts = function (products) {
            ...
            "[object Array]" !== Object.prototype.toString.call(products) &&
              (products = [products]);
            for (var normalizedProducts = [], i = 0; i < products.length; i++) {
              var product = products[i],
                normalizedProduct = {};
              for (_field in (product.id &&
                (isNumber(product.id)
                  ? (normalizedProduct.id = product.id.toString())
                  : isString(product.id) &&
                    (normalizedProduct.id = product.id)),
              product.name &&
                isString(product.name) &&
                (normalizedProduct.name = product.name),
              product.category &&
            ...
            return 0 !== normalizedProducts.length
          ...
          normalizeCustomEventName = function (customEventName) {
            ...
            for (
              var index = 0, lengthOut = 0, lengthIn = customEventName.length;
              index < lengthIn && lengthOut < CONSTANTS.CUSTOM_EVENT_NAME_LIMIT;
              ...
              if (
                ...
                56320 == (64512 & customEventName.charCodeAt(index)) &&
                  (index += 1);
            }
    ...
    14: [
      function (require, module, exports) {
        var sha256 = require("crypto-js/sha256"),
          ...
          isValidUuid = function (uuid) {
            return (
              (uuidDigits = replaceAll(uuid, "-")),
              isHex(uuidDigits, 32) &&
                "00000000000000000000000000000000" !== uuidDigits
            );
          },
          normalizeEmail = (exports.normalizeEmail = function (email) {
            return (
              (localPartAndDomain = email.split("@")),
              2 != localPartAndDomain.length
                ? ""
                : ((localPart = localPartAndDomain[0]),
                  (localPartAndDomain[0] = replaceAll(localPart, ".").split(
                    "+"
                  )[0]),
        ...
        exports.hashIfSet = function (value, valueType) {
          ...
          if (!(0, validators[valueType])(value))
            return "email" === valueType
      ...
      { "crypto-js/enc-hex": 2, "crypto-js/sha256": 3 },
    ],
    15: [
      function (require, module, exports) {
        var CONSTANTS = require("../constants");
